...
docker run --rm --privileged -v "$docker_root:/src" -w /src "$image" ./tools/test-sensor.sh
...
docker run --rm --privileged -v "$docker_root:/src" -w /src "$image" ./tools/test-adversarial.sh
